...
'Hidden helper columns used for click actions.
Private Const ACT_LEFT As String = "AZ"
Private Const ACT_STATUS As String = "BA"
Private Const ACT_CUSTOMER As String = "BB"
Private Const ACT_POST As String = "BC"
Private Const ACT_MODEL As String = "BD"
...
Private Sub ApplyAlternatingRows(ByVal rng As Range)
    ...
    For r = 1 To rng.Rows.Count
        If r Mod 2 = 1 Then
            rng.Rows(r).Interior.Color = RGB(248, 248, 248)
        Else
            rng.Rows(r).Interior.Color = RGB(255, 255, 255)
        End If
...
'=========================================================
' QUICK FILTERS - 3 VISIBLE COLUMNS ONLY
'=========================================================

Private Sub BuildQuickFilterSection()
    Dim d As Worksheet
    Set d = ThisWorkbook.Worksheets(DASHBOARD_SHEET)
    d.Range("B6:D18").ClearContents
    StyleSectionTitle d.Range("B6:D6"), "QUICK FILTERS", RGB(47, 117, 181)
    d.Range("B7").Value = "CATEGORY": d.Range("C7").Value = "COUNT": d.Range("D7").Value = "STATUS"
    StyleDarkHeader d.Range("B7:D7")

    d.Range("B8").Value = "All Records": d.Range("D8").Value = "Total"
    d.Range("B9").Value = "In Service": d.Range("D9").Value = "Complete"
    d.Range("B10").Value = "Not In Service": d.Range("D10").Value = "Review"
    d.Range("B11").Value = "VIN Assigned": d.Range("D11").Value = "OK"
    d.Range("B12").Value = "VIN Pending / Serial Only": d.Range("D12").Value = "Review"
    d.Range("B13").Value = "Needs Review": d.Range("D13").Value = "Action"
    d.Range("B14").Value = "Duplicate VIN": d.Range("D14").Value = "Action"
    d.Range("B15").Value = "Missing Customer": d.Range("D15").Value = "Review"
    d.Range("B16").Value = "Missing Requested Delivery Date": d.Range("D16").Value = "Review"
    d.Range("B17").Value = "Closed": d.Range("D17").Value = "Complete"
    d.Range("B18").Value = "Open": d.Range("D18").Value = "Review"

    d.Range("B8:D8").Interior.Color = RGB(242, 242, 242)
    d.Range("B9:D9").Interior.Color = RGB(226, 239, 218)
    d.Range("B10:D10").Interior.Color = RGB(255, 242, 204)
    d.Range("B11:D11").Interior.Color = RGB(226, 239, 218)
    d.Range("B12:D12").Interior.Color = RGB(255, 242, 204)
    d.Range("B13:D14").Interior.Color = RGB(244, 204, 204)
    d.Range("B15:D16").Interior.Color = RGB(255, 242, 204)
    d.Range("B17:D17").Interior.Color = RGB(198, 239, 206)
    d.Range("B18:D18").Interior.Color = RGB(255, 235, 204)
End Sub

Public Sub RefreshDashboardCounts()
    Dim m As Worksheet, d As Worksheet, lr As Long, r As Long
    Dim cVIN As Long, cVINStatus As Long, cService As Long, cReview As Long, cCustomer As Long, cRDD As Long, cClose As Long
    Dim includeRow As Boolean, viewName As String, s As String
    Dim totalCount As Long, inServiceCount As Long, notServiceCount As Long, assignedCount As Long, pendingCount As Long, reviewCount As Long
    Dim missingCustomerCount As Long, missingRDDCount As Long, closedTotal As Long, openTotal As Long, duplicateCount As Long
    Dim dict As Object, k As Variant

    Set m = ThisWorkbook.Worksheets(MASTER_SHEET)
    Set d = ThisWorkbook.Worksheets(DASHBOARD_SHEET)
    lr = LastMasterRow()
    cVIN = HeaderColumn(m, "VIN")
    cVINStatus = VINStatusColumn(m)
    cService = HeaderColumn(m, "In-Service Status")
    cReview = HeaderColumn(m, "Review Status")
    cCustomer = HeaderColumn(m, "Customer")
    cRDD = HeaderColumn(m, "Requested Delivery Date")
    cClose = HeaderColumnAny(m, "Close", "CLOSE")
    viewName = GetCloseView()
    Set dict = CreateObject("Scripting.Dictionary"): dict.CompareMode = vbTextCompare

    For r = FIRST_DATA_ROW To lr
        If cClose > 0 Then
            If CloseValueIsClosed(m.Cells(r, cClose).Value) Then closedTotal = closedTotal + 1 Else openTotal = openTotal + 1
            includeRow = RowPassesCloseView(m.Cells(r, cClose).Value, viewName)
        Else
            includeRow = True
        End If
        ...
        If includeRow Then
            ...
            If cService > 0 Then
                s = UCase$(CleanText(m.Cells(r, cService).Value))
                If s = "IN SERVICE" Then inServiceCount = inServiceCount + 1
                If s = "NOT IN SERVICE" Then notServiceCount = notServiceCount + 1
            End If
            If cVINStatus > 0 Then
                s = UCase$(CleanText(m.Cells(r, cVINStatus).Value))
                If s = "ASSIGNED" Then assignedCount = assignedCount + 1
                If s = "SERIAL ONLY" Or s = "VIN PENDING" Or s = "INVALID IDENTIFIER" Then pendingCount = pendingCount + 1
            End If
            ...
            If cVIN > 0 Then
                s = CleanText(m.Cells(r, cVIN).Value)
    ...
    For Each k In dict.Keys
        If dict(k) > 1 Then duplicateCount = duplicateCount + dict(k)
    Next k

    d.Range("C8").Value = totalCount
    d.Range("C9").Value = inServiceCount
    d.Range("C10").Value = notServiceCount
    d.Range("C11").Value = assignedCount
    d.Range("C12").Value = pendingCount
    d.Range("C13").Value = reviewCount
    d.Range("C14").Value = duplicateCount
    d.Range("C15").Value = missingCustomerCount
    d.Range("C16").Value = missingRDDCount
    d.Range("C17").Value = closedTotal
    d.Range("C18").Value = openTotal
End Sub

'=========================================================
' LEFT STACK - PROGRAM / YEAR / BRAND / BODY VENDOR
'=========================================================
...
Private Function BuildLeftSection(ByVal StartRow As Long, ByVal SectionTitle As String, ByVal MasterHeader As String, ByVal TypeText As String, ByVal titleColor As Long, Optional ByVal NumericSort As Boolean = False, Optional ByVal ShowPercent As Boolean = False) As Long
    Dim m As Worksheet, d As Worksheet, c As Long, cClose As Long, lr As Long, r As Long
    Dim outRow As Long, total As Long, dict As Object, s As String, Keys As Variant, i As Long

    Set m = ThisWorkbook.Worksheets(MASTER_SHEET)
    Set d = ThisWorkbook.Worksheets(DASHBOARD_SHEET)
    c = HeaderColumn(m, MasterHeader)
    If c = 0 Then BuildLeftSection = StartRow + 3: Exit Function
    cClose = HeaderColumnAny(m, "Close", "CLOSE")
    lr = LastMasterRow()
    Set dict = CreateObject("Scripting.Dictionary"): dict.CompareMode = vbTextCompare
    ...
    For r = FIRST_DATA_ROW To lr
        If cClose = 0 Or RowPassesCloseView(m.Cells(r, cClose).Value, GetCloseView()) Then
            total = total + 1
            s = CleanText(m.Cells(r, c).Value)
            If s <> "" Then If dict.Exists(s) Then dict(s) = dict(s) + 1 Else dict.Add s, 1
        End If
    Next r

    StyleSectionTitle d.Range("B" & StartRow & ":D" & StartRow), SectionTitle, titleColor
    d.Cells(StartRow + 1, "B").Value = MasterHeader
    d.Cells(StartRow + 1, "C").Value = "COUNT"
    If ShowPercent Then d.Cells(StartRow + 1, "D").Value = "% OF TOTAL" Else d.Cells(StartRow + 1, "D").Value = "TYPE"
    StyleDarkHeader d.Range("B" & (StartRow + 1) & ":D" & (StartRow + 1))

    outRow = StartRow + 2
    If dict.Count > 0 Then
        Keys = dict.Keys
        SortKeys Keys, NumericSort
        For i = 0 To dict.Count - 1
            d.Cells(outRow + i, "B").Value = Keys(i)
            d.Cells(outRow + i, "C").Value = dict(Keys(i))
            If ShowPercent Then
                If total > 0 Then d.Cells(outRow + i, "D").Value = dict(Keys(i)) / total
            Else
                d.Cells(outRow + i, "D").Value = TypeText
            End If
            d.Cells(outRow + i, ACT_LEFT).Value = MasterHeader & " = " & Keys(i)
        Next i
        If ShowPercent Then d.Range("D" & outRow & ":D" & (outRow + dict.Count - 1)).NumberFormat = "0.0%"
        ApplyAlternatingRows d.Range("B" & outRow & ":D" & (outRow + dict.Count - 1))
    End If

    BuildLeftSection = outRow + Application.Max(dict.Count, 1) + 2
End Function
...
Public Sub RefreshLeftStack()
    ...
    Set d = ThisWorkbook.Worksheets(DASHBOARD_SHEET)
    On Error Resume Next
    d.Range("B20:D1200").UnMerge
    On Error GoTo 0
    d.Range("B20:D1200").ClearContents
    d.Range("B20:D1200").Interior.Pattern = xlNone
    d.Range(ACT_LEFT & "20:" & ACT_LEFT & "1200").ClearContents
...
'=========================================================
' RIGHT-SIDE 3-COLUMN TABLES
'=========================================================

Private Sub RefreshThreeColumnSection(ByVal FirstCol As String, ByVal LastCol As String, ByVal ActionHelperCol As String, ByVal MasterHeader As String, ByVal SectionTitle As String, ByVal TypeText As String, ByVal BlankLabel As String)
    Dim m As Worksheet, d As Worksheet, c As Long, cClose As Long, lr As Long, r As Long
    Dim blankCount As Long, dict As Object, s As String, Keys As Variant, i As Long, firstColNum As Long
    ...
    On Error Resume Next
    d.Range(FirstCol & "6:" & LastCol & "1200").UnMerge
    On Error GoTo 0
    d.Range(FirstCol & "6:" & LastCol & "1200").ClearContents
    d.Range(FirstCol & "6:" & LastCol & "1200").Interior.Pattern = xlNone
    d.Range(ActionHelperCol & "6:" & ActionHelperCol & "1200").ClearContents

    StyleSectionTitle d.Range(FirstCol & "6:" & LastCol & "6"), SectionTitle, RGB(47, 117, 181)
    firstColNum = d.Range(FirstCol & "1").Column
    d.Cells(7, firstColNum).Value = MasterHeader
    d.Cells(7, firstColNum + 1).Value = "COUNT"
    d.Cells(7, firstColNum + 2).Value = "TYPE"
    StyleDarkHeader d.Range(FirstCol & "7:" & LastCol & "7")

    d.Cells(8, firstColNum).Value = BlankLabel
    d.Cells(8, firstColNum + 1).Value = blankCount
    d.Cells(8, firstColNum + 2).Value = "Review"
    d.Cells(8, ActionHelperCol).Value = MasterHeader & " = [BLANK]"
    With d.Range(FirstCol & "8:" & LastCol & "8")
        .Interior.Color = RGB(255, 242, 204)
    ...
    If dict.Count > 0 Then
        ...
        SortKeys Keys
        For i = 0 To dict.Count - 1
            d.Cells(9 + i, firstColNum).Value = Keys(i)
            d.Cells(9 + i, firstColNum + 1).Value = dict(Keys(i))
            d.Cells(9 + i, firstColNum + 2).Value = TypeText
            d.Cells(9 + i, ActionHelperCol).Value = MasterHeader & " = " & Keys(i)
        Next i
        ApplyAlternatingRows d.Range(d.Cells(9, firstColNum), d.Cells(8 + dict.Count, firstColNum + 2))
    End If
End Sub

Public Sub RefreshStatusSection(): RefreshThreeColumnSection "F", "H", ACT_STATUS, "Order / Delivery Stage", "STATUS / ORDER STAGE", "Stage", "BLANK / NEEDS STATUS": End Sub
Public Sub RefreshCustomerSection(): RefreshThreeColumnSection "J", "L", ACT_CUSTOMER, "Customer", "CUSTOMER", "Customer", "BLANK / MISSING CUSTOMER": End Sub
Public Sub RefreshPostDeliverySection(): RefreshThreeColumnSection "N", "P", ACT_POST, "Post Delivery", "POST DELIVERY", "Post Delivery", "BLANK / NEEDS POST DELIVERY": End Sub
Public Sub RefreshModelSection(): RefreshThreeColumnSection "R", "T", ACT_MODEL, "Model", "MODEL", "Model", "BLANK / MISSING MODEL": End Sub
...
Public Function HandleQuickFilterClick(ByVal Target As Range) As Boolean
    If Intersect(Target, ThisWorkbook.Worksheets(DASHBOARD_SHEET).Range("B8:D18")) Is Nothing Then Exit Function
...
Public Function HandleLeftStackClick(ByVal Target As Range) As Boolean
    ...
    If Target.Row < 20 Or Target.Column < 2 Or Target.Column > 4 Then Exit Function
    actionText = CleanText(ThisWorkbook.Worksheets(DASHBOARD_SHEET).Cells(Target.Row, ACT_LEFT).Value)
    If InStr(1, actionText, " = ") = 0 Then Exit Function
    FilterDashboardAction actionText
    HandleLeftStackClick = True
End Function

Public Function HandleHiddenActionClick(ByVal Target As Range, ByVal FirstCol As Long, ByVal LastCol As Long, ByVal HelperCol As String, Optional ByVal FirstDataRow As Long = 8) As Boolean
    Dim actionText As String
    ...
    actionText = CleanText(ThisWorkbook.Worksheets(DASHBOARD_SHEET).Cells(Target.Row, HelperCol).Value)
    If InStr(1, actionText, " = ") = 0 Then Exit Function
    FilterDashboardAction actionText
    HandleHiddenActionClick = True
End Function
...
Public Function DashboardNeedsRepair() As Boolean
    ...
    If UCase$(CleanText(d.Range("F6").Value)) <> "STATUS / ORDER STAGE" Then DashboardNeedsRepair = True: Exit Function
    If UCase$(CleanText(d.Range("J6").Value)) <> "CUSTOMER" Then DashboardNeedsRepair = True: Exit Function
    If UCase$(CleanText(d.Range("N6").Value)) <> "POST DELIVERY" Then DashboardNeedsRepair = True: Exit Function
    If UCase$(CleanText(d.Range("R6").Value)) <> "MODEL" Then DashboardNeedsRepair = True: Exit Function
...
Public Sub RebuildDashboardLayout()
    ...
    On Error Resume Next
    d.Range("B2:T1200").UnMerge
    On Error GoTo SafeExit
    d.Range("B2:T1200").Clear
    d.Range("AZ:BD").ClearContents

    '3-column compact layout with a spacer between tables.
    d.Columns("B").ColumnWidth = 25
    d.Columns("C").ColumnWidth = 10
    d.Columns("D").ColumnWidth = 14
    d.Columns("E").ColumnWidth = 2
    d.Columns("F").ColumnWidth = 28
    d.Columns("G").ColumnWidth = 10
    d.Columns("H").ColumnWidth = 13
    d.Columns("I").ColumnWidth = 2
    d.Columns("J").ColumnWidth = 30
    d.Columns("K").ColumnWidth = 10
    d.Columns("L").ColumnWidth = 13
    d.Columns("M").ColumnWidth = 2
    d.Columns("N").ColumnWidth = 28
    d.Columns("O").ColumnWidth = 10
    d.Columns("P").ColumnWidth = 13
    d.Columns("Q").ColumnWidth = 2
    d.Columns("R").ColumnWidth = 27
    d.Columns("S").ColumnWidth = 10
    d.Columns("T").ColumnWidth = 13

    SafeMerge d.Range("B2:D3")
    d.Range("B2").Value = "LIVE VIN DASHBOARD — Interactive Navigation"
    With d.Range("B2:D3")
        .Interior.Color = RGB(18, 50, 76)
        .Font.Color = vbWhite
        .Font.Bold = True
    ...
    SafeMerge d.Range("F2:H2")
    d.Range("F2").Value = "HOW TO USE"
    With d.Range("F2:H2")
        .Interior.Color = RGB(221, 235, 247)
        .Font.Color = RGB(18, 50, 76)
        .Font.Bold = True
    End With

    SafeMerge d.Range("F3:H4")
    d.Range("F3").Value = "Click any dashboard row to filter Vehicle Master. Use SHOW RECORDS to choose Open, Closed, or All records."
    With d.Range("F3:H4")
...
Public Sub RefreshDashboardFast()
    ...
    RefreshDashboardCounts
    RefreshAllDynamicSections
    ThisWorkbook.Worksheets(DASHBOARD_SHEET).Columns("AZ:BD").Hidden = True
SafeExit:
    Application.Calculation = oldCalc
    Application.ScreenUpdating = True
    Application.EnableEvents = True
...
Public Sub SetupQuickLookupSheet()
    ...
    SafeMerge q.Range("A1:D2")
    q.Range("A1").Value = "VIN SEARCH & FILTER CENTER"
    With q.Range("A1:D2")
        .Interior.Color = RGB(18, 50, 76)
        .Font.Color = vbWhite
        .Font.Bold = True
    ...
    q.Range("A4").Value = "SHOW RECORDS"
    q.Range(QL_CLOSE).Value = GetCloseView()
    ...
    SafeMerge q.Range("A6:B6")
    q.Range("A6").Value = "FILTER SELECTORS"
    q.Range("A7").Value = "Customer"
    q.Range("A8").Value = "Model"
    q.Range("A9").Value = "Program of Concession"
    q.Range("A10").Value = "Year"
    ...
    SafeMerge q.Range("A15:B15")
    q.Range("A15").Value = "QUICK LOOKUPS"
    q.Range("A16").Value = "Customer text"
    q.Range("A18").Value = "VIN / partial VIN"
    q.Range("A20").Value = "Deal / Reference"
    ...
    With q.Range("A6:B6,A15:B15")
        .Interior.Color = RGB(221, 235, 247)
        .Font.Color = RGB(18, 50, 76)
        .Font.Bold = True
    End With
    q.Range("B7:B10,B16,B18,B20").Interior.Color = RGB(255, 242, 204)
    With q.Range("A12:B13,A22:B22,A24:B24")
        .Interior.Color = RGB(248, 231, 218)
        .Font.Color = RGB(192, 57, 43)
        .Font.Bold = True
    End With
    q.Columns("A").ColumnWidth = 29
    q.Columns("B").ColumnWidth = 38
...
Private Sub BuildQuickLookupValidationList(ByVal MasterHeader As String, ByVal TargetCell As String, ByVal HelperColumn As String, Optional ByVal NumericSort As Boolean = False)
    Dim m As Worksheet, q As Worksheet, c As Long, lr As Long, r As Long
    Dim d As Object, s As String, Keys As Variant, i As Long, lastListRow As Long
    Set m = ThisWorkbook.Worksheets(MASTER_SHEET)
    Set q = ThisWorkbook.Worksheets(LOOKUP_SHEET)
    c = HeaderColumn(m, MasterHeader)
    If c = 0 Then Exit Sub
    ...
    q.Range(HelperColumn & "1:" & HelperColumn & "2000").ClearContents
    q.Range(HelperColumn & "1").Value = "All"
    If d.Count > 0 Then
        Keys = d.Keys
        SortKeys Keys, NumericSort
        For i = 0 To d.Count - 1
            q.Range(HelperColumn & (i + 2)).Value = Keys(i)
        Next i
    End If
    lastListRow = d.Count + 1
    If CleanText(q.Range(TargetCell).Value) = "" Then q.Range(TargetCell).Value = "All"
    On Error Resume Next
    q.Range(TargetCell).Validation.Delete
    On Error GoTo 0
...
Public Sub RefreshQuickLookupLists()
    ...
    BuildQuickLookupValidationList "Customer", QL_CUSTOMER, "AA"
    BuildQuickLookupValidationList "Model", QL_MODEL, "AB"
    BuildQuickLookupValidationList "Program of Concession", QL_PROGRAM, "AC"
    BuildQuickLookupValidationList "Year", QL_YEAR, "AD", True
    q.Columns("AA:AD").Hidden = True
End Sub

Public Sub ApplyQuickLookupFilters()
    Dim q As Worksheet, ws As Worksheet, d As Worksheet, lr As Long, lc As Long, c As Long
    Dim labelText As String, customerValue As String, modelValue As String, programValue As String, yearValue As String
    Set q = ThisWorkbook.Worksheets(LOOKUP_SHEET)
    Set ws = ThisWorkbook.Worksheets(MASTER_SHEET)
    Set d = ThisWorkbook.Worksheets(DASHBOARD_SHEET)

    d.Range(CLOSE_SELECTOR_CELL).Value = q.Range(QL_CLOSE).Value
    customerValue = CleanText(q.Range(QL_CUSTOMER).Value)
    modelValue = CleanText(q.Range(QL_MODEL).Value)
    programValue = CleanText(q.Range(QL_PROGRAM).Value)
    yearValue = CleanText(q.Range(QL_YEAR).Value)
    lr = LastMasterRow(): lc = LastMasterColumn(): PrepareMasterFilter

    If customerValue <> "" And UCase$(customerValue) <> "ALL" Then
        c = HeaderColumn(ws, "Customer")
        If c > 0 Then ws.Range(ws.Cells(HEADER_ROW, 1), ws.Cells(lr, lc)).AutoFilter Field:=c, Criteria1:=customerValue: labelText = "Customer = " & customerValue
    End If
    If modelValue <> "" And UCase$(modelValue) <> "ALL" Then
        c = HeaderColumn(ws, "Model")
        If c > 0 Then
            ws.Range(ws.Cells(HEADER_ROW, 1), ws.Cells(lr, lc)).AutoFilter Field:=c, Criteria1:=modelValue
            If labelText <> "" Then labelText = labelText & " | "
            labelText = labelText & "Model = " & modelValue
        End If
    End If
    If programValue <> "" And UCase$(programValue) <> "ALL" Then
        c = HeaderColumn(ws, "Program of Concession")
        If c > 0 Then
            ws.Range(ws.Cells(HEADER_ROW, 1), ws.Cells(lr, lc)).AutoFilter Field:=c, Criteria1:=programValue
            If labelText <> "" Then labelText = labelText & " | "
            labelText = labelText & "Program = " & programValue
        End If
    End If
    If yearValue <> "" And UCase$(yearValue) <> "ALL" And IsNumeric(yearValue) Then
        c = HeaderColumn(ws, "Year")
        If c > 0 Then
            ws.Range(ws.Cells(HEADER_ROW, 1), ws.Cells(lr, lc)).AutoFilter Field:=c, Criteria1:=CLng(yearValue)
            If labelText <> "" Then labelText = labelText & " | "
            labelText = labelText & "Year = " & yearValue
        End If
    End If

    ApplyCloseViewFilter ws, lr, lc
    If labelText = "" Then labelText = "All Records"
    SetActiveFilterLabel labelText
    ws.Activate
End Sub

Public Sub RunQuickLookup()
    Dim q As Worksheet, ws As Worksheet, d As Worksheet, lr As Long, lc As Long, c As Long
    Dim labelText As String, customerText As String, vinText As String, dealText As String
    Set q = ThisWorkbook.Worksheets(LOOKUP_SHEET)
    Set ws = ThisWorkbook.Worksheets(MASTER_SHEET)
    Set d = ThisWorkbook.Worksheets(DASHBOARD_SHEET)

    d.Range(CLOSE_SELECTOR_CELL).Value = q.Range(QL_CLOSE).Value
    customerText = CleanText(q.Range(QL_CUSTOMER_SEARCH).Value)
    vinText = CleanText(q.Range(QL_VIN_SEARCH).Value)
    dealText = CleanText(q.Range(QL_DEAL_SEARCH).Value)
    If customerText = "" And vinText = "" And dealText = "" Then
        MsgBox "Enter a Customer, VIN, or Deal / Reference.", vbInformation, "VIN Manager"
        Exit Sub
    End If
    ...
    If customerText <> "" Then
        c = HeaderColumn(ws, "Customer")
        If c > 0 Then ws.Range(ws.Cells(HEADER_ROW, 1), ws.Cells(lr, lc)).AutoFilter Field:=c, Criteria1:="*" & customerText & "*": labelText = "Customer contains " & customerText
    End If
    If vinText <> "" Then
        c = HeaderColumn(ws, "VIN")
        If c > 0 Then
            ws.Range(ws.Cells(HEADER_ROW, 1), ws.Cells(lr, lc)).AutoFilter Field:=c, Criteria1:="*" & vinText & "*"
            If labelText <> "" Then labelText = labelText & " | "
            labelText = labelText & "VIN contains " & vinText
        End If
    End If
    If dealText <> "" Then
        c = HeaderColumn(ws, "Deal / Reference")
        If c > 0 Then
            ws.Range(ws.Cells(HEADER_ROW, 1), ws.Cells(lr, lc)).AutoFilter Field:=c, Criteria1:="*" & dealText & "*"
            If labelText <> "" Then labelText = labelText & " | "
            labelText = labelText & "Deal contains " & dealText
        End If
    End If
...
Public Sub ResetQuickLookup()
    ...
    Set q = ThisWorkbook.Worksheets(LOOKUP_SHEET)
    Set d = ThisWorkbook.Worksheets(DASHBOARD_SHEET)
    q.Range(QL_CLOSE).Value = "All"
    q.Range(QL_CUSTOMER).Value = "All"
    q.Range(QL_MODEL).Value = "All"
    q.Range(QL_PROGRAM).Value = "All"
    q.Range(QL_YEAR).Value = "All"
    q.Range(QL_CUSTOMER_SEARCH).ClearContents
    q.Range(QL_VIN_SEARCH).ClearContents
    q.Range(QL_DEAL_SEARCH).ClearContents
...
Public Sub GoToQuickLookup()
    SetupQuickLookupSheet
    ThisWorkbook.Worksheets(LOOKUP_SHEET).Activate
End Sub
